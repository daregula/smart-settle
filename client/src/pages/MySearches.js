...
export const MySearches = (props) => {
    ...
    const fetchSavedResponses = useCallback(async () => {
        try {
            const response = await axios.get(`https://smart-settle-api.vercel.app/responses/savedResponses/ids/${userOwner}`);
    ...
    const deleteResponse = async (RID) => {
        try {
            await axios.delete(`https://smart-settle-api.vercel.app/responses/deleteResponse/ids/${RID}`).then(() => {
